...
$( () => {
				...
				*/
				...
				switch (certificates[i].type) {
					case 'value':
						...
						remainingValue = `$${parseFloat(certificates[i].remainingValue).toLocaleString().toFixed(2)}`;
						remainingType = null;
						break;
					case 'count':
	...
	$('#buy_single_class_submit').on('click', async (e) => {
		...
		productURL += `&firstName=${firstName}&lastName=${lastName}&email=${email}&phone=${phone}&certificate=${email}`;
